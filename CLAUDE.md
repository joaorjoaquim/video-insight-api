...
Admin routes: `X-Admin-Hash` header, compared with `ADMIN_HASH` env var using timing-safe compare (`crypto.timingSafeEqual` + SHA-256). Rate-limited 10 req/15 min per IP.
...
```json
{ "action": "star" | "fork", "repo": "web" | "api", "githubUsername": "..." }
```

`repo` defaults to `"web"` if omitted (frontend currently only targets the web repo).
`githubUsername` optional if already stored on the user from a prior claim or GitHub OAuth.
...
- **GitHub OAuth button missing in frontend** — the API supports `GET /auth/oauth/github` but the AuthDialog in the web app has no GitHub button. Users can only link GitHub via the claim form (manual username entry).
- **Frontend only claims web repo** — the wallet UI has no way to claim credits for the API repo. The API supports `repo: "api"` but the frontend always omits `repo` (defaults to `"web"`).
- **Payment integration** — the wallet "Buy Credits" dialog is UI-only; no payment processor is wired (Stripe, etc.).
